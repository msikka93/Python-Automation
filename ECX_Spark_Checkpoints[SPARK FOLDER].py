...
def getConfiguration():
    while True:
        try:
            ...
            response = requests.get("http://odh.lgi.io/marathon/v2/apps//spark/{0}".format(spark_job_name),
...
def getCheckPointPath(commandOutputList):
    ...
    for checkpoint in commandOutputList:
        if "checkpoint:" in checkpoint:
            ...
            pass
    checkpoint_path_retrieved = checkpoints[0].split(': ')[1][:-1]+"0"
...
def get_spark_checkpoints_list(commandOutputList):
    ...
    for checkpoints in commandOutputList:
        if "spark_checkpoints" in checkpoints:
...
def ssh_conn():
    ...
    while True:
        try:
            ...
            client.connect('172.23.112.20',port =22, username='gsikka', pkey=k)
