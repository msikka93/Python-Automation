...
class Sparkjob:
    ...
    def update_spark_instances(self,replace_spark_config,job_name):
        try:
            ...
            response = requests.put("http://connectivity.odh.lgi.io/marathon/v2/apps//spark/{0}".format(job_name),
                            json=payload,
    ...
    def get_spark_properties(self):
        while True:
            try:
                ...
                response = requests.get("http://connectivity.odh.lgi.io/marathon/v2/apps//spark/{0}".format(job_name),
                                        auth=('odhuser', 'password'))
                data = response.json()
...
#connect-app-cdm-log-parsing-pl