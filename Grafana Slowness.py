...
def ssh_conn(hostname):
    print(f"{'#' * 50} Python Script For restarting carbon docker containers on {hostname} {'#' * 50}")
    while True:
        try:
            ...
            client.connect(hostname,port =22, username= 'gsikka', pkey=k)
...
thread1 = threading.Thread(target=ssh_conn,
                           args=(['172.23.112.45']))

thread2 = threading.Thread(target=ssh_conn,
                           args=(['172.23.112.50']))
...
thread2.join()
sys.exit()
#172.23.112.45 --odhecx22e
#172.23.112.50 --odhecx27e
